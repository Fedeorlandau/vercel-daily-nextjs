...
export function TrendingArticles({ articles, onClick }: TrendingArticlesProps) {
  return (
    <aside>
      <div className="border-b-2 border-foreground pb-2 mb-4">
        <p className="label-mono text-accent text-xs mb-0.5">◆ TRENDING</p>
        <h3 className="font-serif font-bold text-lg text-foreground">You May Also Like</h3>
      </div>
        ...
        {articles.map((article, idx) => (
          ...
          >
            ...
            <div>
              ...
              <p className="label-mono text-xs text-muted-foreground mt-1">{article.date}</p>
            </div>
          </li>
        ))}
      </ol>
